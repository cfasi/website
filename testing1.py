...
ALL_ROLES = ["team_member", "trainer", "shift_lead", "supervisor", "director", "operator"]
...
def is_sop_category(category: str) -> bool:
    return category == SOP_CATEGORY


def can_edit_sop_resources() -> bool:
    """
    Everyone except Team Members may add, edit, and delete
    resources on the Standard Operating Procedures page.
    """
...
def can_request_links_in_category(category: str) -> bool:
    # SOP submissions are posted directly, not submitted for approval.
    if is_sop_category(category):
        return False

    return (
        get_current_role() in ["trainer", "shift_lead"]
        and user_owns_category(category)
    )


def can_post_directly_in_category(category: str) -> bool:
    # All ranks except Team Members may post directly to SOP.
    if is_sop_category(category):
        return can_edit_sop_resources()

    if get_current_role() == "director":
        return True

    return (
        get_current_role() == "supervisor"
        and user_owns_category(category)
    )
...
def can_manage_links_in_category(category: str) -> bool:
    # All ranks except Team Members may edit/delete SOP resources.
    if is_sop_category(category):
        return can_edit_sop_resources()
...
def get_allowed_view_roles_for_poster(category: str):
    ...
    # Anyone permitted to edit SOP may choose the minimum viewing rank.
...
def create_link_submission(
    ...
    try:
        ...
        st.error(f"Error creating resource: {e}")
        return False
...
def clean_storage_filename(filename: str) -> str:
    """
    Removes characters that may cause problems in a Storage path.
    """
...
def upload_resource_file(uploaded_file, category: str):
    ...
    try:
        ...
        supabase.storage.from_(SOP_STORAGE_BUCKET).upload(
            path=storage_path,
            file=file_bytes,
            file_options={
        ...
        raise RuntimeError(
            "The file uploaded, but its public URL could not be retrieved."
        )
...
def get_user_favorites():
    """
    Returns all Favorites belonging only to the currently logged-in user.
    """
    try:
        result = (
            supabase.table("user_favorites")
            .select(
                "id, username, resource_link_id, custom_name, "
                "custom_url, custom_description, created_at, "
                "resource_links("
                "id, category, system_name, description, "
                "external_url, file_url, visible, status, min_view_role"
                ")"
            )
            .eq("username", get_current_username())
            .order("created_at", desc=True)
            .execute()
        )
        ...
        for favorite in favorites:
            resource = favorite.get("resource_links")

            # Personal/custom favorite
            if favorite.get("resource_link_id") is None:
                visible_favorites.append(favorite)
                continue

            # Existing website resource favorite
            if not resource:
                continue

            # Do not show resources that were hidden or unapproved later
            if resource.get("status") != "approved":
                continue

            if not resource.get("visible", True):
                continue

            # Continue respecting the resource's minimum viewing role
            if not can_user_view_link(resource):
                continue

            visible_favorites.append(favorite)

        return visible_favorites
...
def is_resource_favorited(resource_link_id: int) -> bool:
    """
    Checks whether the current user already favorited a website resource.
    """
    try:
        result = (
            supabase.table("user_favorites")
            .select("id")
            .eq("username", get_current_username())
            .eq("resource_link_id", resource_link_id)
            .limit(1)
            .execute()
        )
...
def add_resource_favorite(resource_link_id: int) -> bool:
    """
    Favorites an existing resource from the website.
    """
    try:
        if is_resource_favorited(resource_link_id):
            return True
...
def add_custom_favorite(
    ...
    """
    Creates a private personal link visible only to the current user.
    """
    try:
        result = (
            supabase.table("user_favorites")
        ...
        st.error(f"Error adding personal link: {e}")
        return False


def remove_favorite(favorite_id: int) -> bool:
    """
    Removes a favorite while verifying that it belongs to the current user.
    """
    try:
        (
            supabase.table("user_favorites")
...
def remove_resource_favorite(resource_link_id: int) -> bool:
    """
    Removes an existing website resource from the current user's Favorites.
    """
    try:
        (
            supabase.table("user_favorites")
...
def show_link_card(link, show_actions=False):
    ...
    favorited = is_resource_favorited(link["id"])

    if favorited:
        if st.button(
            "★ Remove from Favorites",
            key=f"unfavorite_resource_{link['id']}",
            use_container_width=True
        ):
            if remove_resource_favorite(link["id"]):
                st.success("Removed from Favorites.")
                st.rerun()
    else:
        if st.button(
            "☆ Add to Favorites",
            key=f"favorite_resource_{link['id']}",
            use_container_width=True
        ):
            if add_resource_favorite(link["id"]):
...
def render_links_section(category: str):
    ...
    search_query = st.text_input(
        ...
        key=f"resource_search_{category}"
    )

    filtered_links = approved_links

    if search_query.strip():
        keyword = search_query.strip().lower()

        filtered_links = [
            link
            for link in approved_links
            if (
                keyword in (link.get("system_name") or "").lower()
                or keyword in (link.get("description") or "").lower()
            )
        ]
    ...
    if filtered_links:
        for link in filtered_links:
            show_link_card(
                link,
                show_actions=(edit_mode and can_manage_here)
            )
    elif approved_links and search_query.strip():
        st.info("No resources match your search.")
    else:
        st.info("No resources have been posted yet for this category.")
    ...
    if edit_mode and (can_submit_request or can_submit_direct):
        ...
        with st.form(
            f"submit_link_form_{category}",
            clear_on_submit=True
        ):
            system_name = st.text_input(
                "Resource Name",
                key=f"link_system_name_{category}"
            )

            description = st.text_area(
                "Description",
                key=f"link_description_{category}"
            )
        ...
        if submit_link:
            resource_name = system_name.strip()
            entered_url = external_url.strip()
            ...
            if not resource_name:
                ...
                if success:
                    ...
                    st.error("Unable to submit resource.")
...
def render_user_management():
    ...
    if login_history:
        ...
        if "login_time" in login_df.columns:
            login_df["login_time"] = pd.to_datetime(
                login_df["login_time"],
                errors="coerce",
                utc=True
            )
        ...
        # Make role names easier to read
        if "role" in login_df.columns:
            login_df["role"] = login_df["role"].map(
                lambda role: ROLE_LABELS.get(role, role)
            )

        # Rename columns for display
        login_df = login_df.rename(
            columns={
        ...
        display_columns = [
            ...
            if column in login_df.columns
        ]

        st.dataframe(
            login_df[display_columns],
            use_container_width=True,
            hide_index=True
        )

    else:
        st.info("No login history has been recorded yet.")
    ...
    if editable_users:
        ...
        if save_user_changes:
            if new_role not in ["director", "team_member", "operator"] and not new_category:
...
def favorites():
    ...
    full_name = get_current_full_name().strip()
    ...
    # -----------------------------------------
    # Add private personal link
    # -----------------------------------------
    with st.expander("＋ Add a Personal Link"):
        st.caption(
            "Personal links are private and will only appear in your account."
        )

        with st.form(
            "add_personal_favorite_form",
            clear_on_submit=True
        ):
            custom_name = st.text_input(
                "Link Name",
                placeholder="Example: HotSchedules"
            )
            ...
            add_personal_link = st.form_submit_button(
                "Add to My Favorites",
                type="primary",
                use_container_width=True
            )

        if add_personal_link:
            entered_name = custom_name.strip()
            entered_url = custom_url.strip()

            if not entered_name:
                st.error("Please enter a link name.")

            elif not entered_url:
                st.error("Please enter a website link.")

            elif not entered_url.lower().startswith(
                ("http://", "https://")
            ):
                st.error(
                    "Please enter a complete link beginning with "
                    "http:// or https://"
                )

            else:
                success = add_custom_favorite(
                    custom_name=entered_name,
                    custom_url=entered_url,
                    custom_description=custom_description
                )

                if success:
                    st.success("Personal link added to Favorites.")
                    st.rerun()
                else:
                    st.error("Unable to add the personal link.")
    ...
    search_query = st.text_input(
        ...
        key="favorites_search"
    )

    all_favorites = get_user_favorites()
        ...
        for favorite in all_favorites:
            ...
            if resource:
                searchable_text = " ".join([
                    resource.get("system_name") or "",
                    resource.get("description") or "",
                    resource.get("category") or ""
                ]).lower()
            else:
                searchable_text = " ".join([
                    favorite.get("custom_name") or "",
                    favorite.get("custom_description") or "",
                    favorite.get("custom_url") or ""
                ]).lower()
    ...
    if not filtered_favorites:
        if all_favorites and search_query.strip():
            st.info("No Favorites match your search.")
        else:
            st.info(
                "You have not added any Favorites yet. "
                "Add a personal link above or select "
                "'Add to Favorites' on a website resource."
            )
        return

    st.markdown(f"### My Favorites ({len(filtered_favorites)})")

    for favorite in filtered_favorites:
        resource = favorite.get("resource_links")

        # -------------------------------------
        # Existing website resource
        # -------------------------------------
        if resource:
            ...
            st.caption(
                f"Website Resource • "
                f"{resource.get('category', 'Uncategorized')}"
            )
            ...
            if external_url:
                st.markdown(
                    f"[Open External Link]({external_url})"
                )

            if file_url:
                st.markdown(
                    f"[Open Uploaded File]({file_url})"
                )
        ...
        # -------------------------------------
            ...
            custom_description = favorite.get(
                "custom_description"
            )

            if custom_description:
                st.write(custom_description)
            else:
                st.write("No description provided.")

            st.caption("Private Personal Link • Only visible to you")

            custom_url = favorite.get("custom_url")

            if custom_url:
                st.markdown(
                    f"[Open Personal Link]({custom_url})"
                )
        ...
        if st.button(
            ...
            if remove_favorite(favorite["id"]):
                st.success("Removed from Favorites.")
                st.rerun()
...
if page == "Home":
    render_home()
elif page == "Standard Operating Procedures":
    render_category_page("Standard Operating Procedures")
